...
function drawTouchFrame(board) {
    // draws a frame of the layer on top of the actual board.
    // used for the grid and on-screen info.
    ctt.clearRect(0, 0, touchCanvas.width, touchCanvas.height);
...
function flashMsg(board, message, displayCycles) {
    // flashes a helpful message about changes, selections etc. Uses own canvas.
...
function persistSelection(board) {
    // checks if there is a selection, and persists it by redrawing it.
...
function drawSnapShotTemplate (board) {
    // draws useful on-screen info for the snapshot, such as the rules.
