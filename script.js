            ...
            binders: {
                pen: function (game, ...args) {
                    $('#touchCanvas').on('mousedown.tool', function (e) {
                        if (e.which === 1) {
                            e.preventDefault();
                            // capture the state of the board before drawing to allow undos
                ...
                grab : function (game) {
                    $('#touchCanvas').css('cursor', 'grab');
                    $('#touchCanvas').on('mousedown.tool', function () {
                        let {x, y} = game.current.mousePos.coords;
                        $('#touchCanvas').css('cursor', 'grabbing');
                        $(this).on('mousemove.tool', function (e) {

                            let wantsToBeAt = { x: x, y: y };
                            let isAt = utils.getEventCoords(e, board, true, false);

                            let xDif = wantsToBeAt.x - isAt.x;
                            let yDif = wantsToBeAt.y - isAt.y;

                            board.settings.translationOffset.x -= xDif;
                            board.settings.translationOffset.y -= yDif;
...
$('.selectGrab').click(() => {
    canvasFuncs.flashMsg(game.current, `Tool : Move`, 15);
    utils.changeTool('grab', game);
});
...
$('#touchCanvas').on('mousemove', function (e) {
    ...
    let xRaw = e.offsetX + 0;
    let yRaw = e.offsetY + 0;

    game.current.mousePos.coords.x = currentCoords.x;
    game.current.mousePos.coords.y = currentCoords.y;
    game.current.mousePos.coords.xRaw = xRaw;
    game.current.mousePos.coords.yRaw = yRaw;
...
$('#touchCanvas').on('mousedown', function (e) {
    $('input').blur();
    console.log(e.which);
...
// hold ctrl for selection or alt for grab...
$(document).on('keydown.shortcuts', function (e) {
    let typing = $('input').is(':focus');

    if (typing) return;
    if (game.tools.current.name === 'pen') {
        let clone = {};
    ...
    if (e.which === 18) {
        $('.selectGrab').trigger('click');
    }
})

// ...switch back to pen tool on keyup.
...
/// ---------- Scroll Zoom ------------ ///
// creates a zoomed in board, then finds which coord
// the mouse is currently on, and which it wants to be on. 
// we then simply translate by the difference.
$('.canvasContainer').bind('wheel', function (e) { 
    let board = game.current;
    let delta = e.originalEvent.wheelDelta;
    let zoomStep = Math.abs(delta) * 0.001;
    let zoomMod = delta > 0 ? 1 + zoomStep : 1 - zoomStep;
    // the mousePos updates only when the mouse (actually) moves
    let { x, y } = game.current.mousePos.coords;
    
    let w = board.spaceWidth;
    let nextW = w * zoomMod
    let minW = window.innerWidth / board.width;
    let maxW = window.innerWidth;
    game.current.spaceWidth = nextW > maxW ? maxW : nextW < minW ? minW : nextW;
    ...
    canvasFuncs.drawTouchFrame(board);
...
$('.toggleMenu').click(() => {
    ...
    $('.slider, .button').toggleClass('dnone');
