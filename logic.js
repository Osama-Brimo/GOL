...
let LtLPresets = {
    ...
    'cartoonBears' : 'R8,C0,M1,S141..184,B15..176,NM',
    'Rope' : 'R6,C0,M0,S80..138,B40..71,NM'
}
...
function loadNextState(board) {
    ...
    // if the previous state cache exceeds the cacheLimit, then clear
    if (board.cache.length > cacheLimit) {
        board.cache = [];
    }

    // cache the current state before calculating the next gen
    ...
    // only check units within the boundary
