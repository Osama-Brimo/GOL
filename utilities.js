...
function determinStepToOrigin(coordA, coordB) {
    // checks if we should move negatively or positively to approach canvas origin
    // for both x and y.
...
function getBoundingBox(coordA, coordB, board) {
    // gets all spaces within a selection
...
function parsePattern(patternTxt) {
    // turns .cells file text into an array of arrays, each array signifying a row
...
function mapPatternToSpace(pattern, board, origin) {
    // Maps the parsed .cells file onto the board by assuming every array lies
    // on one board row.

    // The offset allows the pattern to be loaded with any point as the origin,
    // given as a coord or space.
...
function mapSelectionToOrigin(spaces, board) {
    // returns the selection with as many x's and y's taken out of it
    // as possible (as if its top leftmost space was the origin).

    // used to set the origin of a pattern to any point.
...
function mapSpaceToPattern(spaces, board) {
    // creates a .cells file representation from an array of spaces.
    spaces.sort((a, b) => a - b);
    let result = ``;
...
function generateRandomRuleArray(spread) {
    // returns an array with random numbers representing birth/survive numbers, depending on the spread.
    let result = [];
...
function parseBSRuleString (rulestring) {
    // turns a B/S notation rulestring into the rules it describes.
    let result = {
        birth: [],
        survive: [],
        spread : 1,
...
function parseLtLRulestring (rulestring) {
    // turns an LtL notation rulestring into the rules it describes.
    let result = {
        birth : [],
        survive : [],
        spread : true,
...
function parseRulestring(rulestring) {
    // checks notation, and returns the rule object which it describes.
    let specialWords = ['random', 'default'];
    let ruleFamily = $('#inputBar').attr('rule');
    let cleanRulestring = rulestring.toLowerCase().trim();
    let isSpecial = specialWords.includes(cleanRulestring);
    let isLtL = ruleFamily === 'LtL';

    if (rulestring.length >= 3) {

        if (isSpecial) {
            switch (cleanRulestring) {
                case 'random':
        ...
        let result = !isLtL ? parseBSRuleString(rulestring) : parseLtLRulestring(rulestring);
        return result;
    } else {
...
function generateBSRulestring (rule) {
    // turns a rule object into a B/S notation rulestring.
    let result = ``;
...
function generateLtLRulestring (rule) {
    // turns a rule object into an LtL notation rulestring.
    let ranges = {
...
function generateRulestring(rule) {
    // finds if the rule needs to be described as LtL or as B/S, and returns the rulestring.
...
function determinePatternBoundary(pattern, board, returnSpace) {
    // Determines the rectangular "selection" which would capture every living
    // space for a given pattern. Used to ignore dead portions of the board.
    if (pattern.length === 0) {
...
function matchCoordsToSelection(origin, current) {
    // Selection from each direction starts from the same corner of a space/cell, causing each direction to select differently.
    // This can be fixed by adding 1 unit to every x or y bigger than its counterpart in the other coord.
...
function triggerSliderAtValue(val, sliderName) {
    ...
    // divide by step to find how many valLens the value is worth
    val = (val / sliderStep);

    let numOfVals = Math.round(maxVal / sliderStep);
    let valLen = sliderWidth / numOfVals;
